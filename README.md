...
- **Multi-source ingestion**:
  ...
  - **Aggregator**: **DevJobsScanner** HTML listing scan (`devjobsscanner_scan`)
...
- `devjobsscanner_scan`: optional `promote_companies` (default `false`), `max_promote_per_run`, `listing_urls`, `max_job_details`; optional `brave_api_key` when promotion is enabled.
- `reddit_feed`: sends a descriptive `user_agent` and polite delays to respect Reddit's unauthenticated rate limits.

Pipeline order (`PipelineService.runDaily`): **CF Global Watcher → Discovery → scrape `runAll` (includes company-link enrichment) → score → alerts**.
